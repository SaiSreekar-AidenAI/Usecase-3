...
export class AppStateService {
  private conversationsApi = inject(ConversationsApiService);
  ...
  async generate(): Promise<void> {
    ...
    try {
      const result = await firstValueFrom(
        this.conversationsApi.generateResponse(q, this._customPrompt() || undefined),
      );
  ...
  async loadHistory(): Promise<void> {
    try {
      const items = await firstValueFrom(this.conversationsApi.fetchHistory());
  ...
  async updateConversationResponse(id: string, response: string): Promise<void> {
    await firstValueFrom(this.conversationsApi.updateHistoryItem(id, response));
  ...
  async deleteConversation(id: string): Promise<void> {
    await firstValueFrom(this.conversationsApi.deleteHistoryItem(id));
  ...
  async clearAllHistory(): Promise<void> {
    await firstValueFrom(this.conversationsApi.clearAllHistory());
    this._history.set([]);
    this._selectedConversationId.set(null);
    this.router.navigate(['/generate']);
  }
