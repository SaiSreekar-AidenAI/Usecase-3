  ...
  private async load(): Promise<void> {
    try {
      const [sessions, alerts, logins] = await Promise.all([
        firstValueFrom(this.analyticsApi.fetchSessions()),
        firstValueFrom(this.analyticsApi.fetchSecurityAlerts()),
        firstValueFrom(this.analyticsApi.fetchLoginAttempts({ limit: 20 })),
      ]);
