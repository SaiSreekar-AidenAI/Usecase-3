...
/**
 * Wrapper around fetch that handles JWT token refresh transparently.
 * If the server returns a new JWT in X-Access-Token header, it updates localStorage.
 */
...
export async function updateHistoryItem(id: string, response: string): Promise<void> {
  const res = await apiFetch(`${API_BASE}/api/history/${id}`, {
    method: 'PATCH',
...
export async function updateUserApi(
  ...
  const res = await apiFetch(`${API_BASE}/api/users/${userId}`, {
    method: 'PATCH',
    body: JSON.stringify(data),
  });
  if (!res.ok) {
...
export async function fetchOverviewStats(): Promise<OverviewStats> {
  const res = await apiFetch(`${API_BASE}/api/analytics/overview`);
  if (!res.ok) throw new Error('Failed to fetch overview stats');
  return res.json();
}

export async function fetchDailyActivity(days: number = 30): Promise<DailyActivityPoint[]> {
  const res = await apiFetch(`${API_BASE}/api/analytics/daily-activity?days=${days}`);
...
export async function fetchAuditLog(params: {
  ...
  const res = await apiFetch(`${API_BASE}/api/analytics/audit-log?${qs}`);
...
export async function fetchLoginAttempts(params: {
  ...
  if (params.from_ts) qs.set('from_ts', String(params.from_ts));
  if (params.to_ts) qs.set('to_ts', String(params.to_ts));
  const res = await apiFetch(`${API_BASE}/api/analytics/login-attempts?${qs}`);
  if (!res.ok) throw new Error('Failed to fetch login attempts');
  return res.json();
}

export async function fetchSessions(): Promise<SessionActivity[]> {
  const res = await apiFetch(`${API_BASE}/api/analytics/sessions`);
...
export async function sendHeartbeat(active: boolean): Promise<void> {
  try {
    await apiFetch(`${API_BASE}/api/analytics/heartbeat`, {
      method: 'POST',
